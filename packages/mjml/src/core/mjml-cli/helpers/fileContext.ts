...
const ensureIncludeIsSupportedFile = (file: any) => path.extname(file).match(/\.mjml|\.css|\.html/) ? file : `${file}.mjml`
  
const error = (e: any) => console.error(e.stack || e) // eslint-disable-line no-console

export default (baseFile: any, filePath: any) => {
  const filesIncluded: any = []
  ...
  const readIncludes = (dir: any, file: any, base: any) => {
