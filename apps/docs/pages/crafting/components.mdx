...
<Demo>
  <Email backgroundColor={Colors.grey100}>
    <Container py={16} px={16} backgroundColor="white" my={16}>
      <Row>
        <Column backgroundColor={Colors.grey100} py={16}>
          <Typography textAlign="center" fontWeight="bold">
...
<ContentDemo>
  {[
    ...
    <Typography as={variant} fontSize={size} key={variant}>
      {variant} (size: {size})
    </Typography>
  ))}
</ContentDemo>
...
With that said, Brail provides a [linter](/crafting/linting) which provides warnings about email support for various elements, CSS properties and other features. And as per usual, you can always test and preview your templates to be sure.
