...
export default baseTemplate
  .props(z.object({ firstName: z.string(), pet: z.string() })) /* 👈 Define props meta */
  .preview({ firstName: "John", pet: "Fido" }) /* 👈 Define props used in preview */
  (/* To be continued... */)
```
...
```tsx
  ...
  .view(props => {
		...
		{/* 👇 Code is continued in accordion below */}
```
...
<details>
	<summary>
		...
		**Open for remaining template code**<br/>
		</div>
	</summary>
...
const html = await myTemplate.render(props)
...
export const appRouter = createTrpcRouter({ myTemplate: myTemplate })
